...
function renderLevelSelection() {
    ...
    for (let i = 1; i <= 10; i++) {
        ...
        const isUnlocked = i === 1 || window.progress.levels[`topik12-${i-1}`].mcPassed;
        ...
        card.innerHTML = 
            ...
            '<div class="mode-buttons">' +
                '<button class="mode-btn mc" ' + (isUnlocked ? '' : 'disabled') + ' onclick="startMode(\'topik12\', ' + i + ', \'mc\')">Multiple Choice</button>' +
                '<button class="mode-btn tp" ' + (isUnlocked ? '' : 'disabled') + ' onclick="startMode(\'topik12\', ' + i + ', \'tp\')">Typing Practice</button>' +
            '</div>';
...
function getLevelData(difficulty, level) {
    if (difficulty === 'topik12') {
        return window.TOPIK12_DATA ? window.TOPIK12_DATA[level] : null;
    }
    return null;
}